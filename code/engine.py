...
from utils.util import setup_seed, setup_recsys, print_opt, print_text, save_model, load_model, init_load_model
...
def run():
    ...
    for epoch_idx in range(opt["epochs"]):
        ...
        # evaluate here
        if epoch_idx % 5 == 0:# and epoch_idx != 0
...
if __name__ == "__main__":
    ...
    if opt["sign_ft"] == 1:
        recsys = init_load_model(opt, os.path.join(opt["init_path_ft"],'best_checkpoint_with_assignment_update.pt'))
    ...
    if opt["sign_ft"] == 0:
        ...
        for name, param in recsys.named_parameters():
            if name.find("finetune_embs") != -1:
        ...
        parameters_to_exclude = []
        for name, param in recsys.named_parameters():
            if name.find("centroid_embs") != -1:
                print(name)
                parameters_to_exclude.append(param)
            else:
                parameters_to_update.append(param)
    optimizer = torch.optim.Adam([
        {'params': parameters_to_update},
        {'params': parameters_to_exclude, 'lr': 0.0}
    ], lr=opt["lr"], weight_decay=opt["optimizer_weight_decay"])
