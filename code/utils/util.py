...
def init_load_model(opt: dict, load_path: str) -> FM:
    """
    Load model checkpoint
    """
    if not load_path.endswith(".pt"):
    ...
    # if opt["sign_ft"]==1:
    #     recsys.embedding.clustering.init_finetune()

    import torch.nn as nn
    if opt["sign_ft"] == 1:
        # init after load
        recsys.embedding.clustering.finetune_assignment = recsys.embedding.clustering.random_init_centroid_assignment(
            (opt["num_clusters"], opt["num_clusters_ft"]),
            non_zero_row_count=opt["num_clusters_ft"]).to(opt["device_id"])#opt["top_k_ft"]
        finetune_embs = torch.zeros((opt["num_clusters"], opt["num_clusters_ft"]))
        finetune_embs = nn.init.normal_(finetune_embs)
        recsys.embedding.clustering.finetune_embs = nn.Parameter(finetune_embs)
        recsys.embedding.clustering.num_clusters_ft = opt["num_clusters_ft"]
        recsys.embedding.clustering.num_composition_centroid = opt["num_composition_centroid"]
        recsys.embedding.clustering.init_finetune()
        pass
    #
    # load to correct device
    ...
    clustering = recsys.embedding.clustering
    clustering.centroid_assignment = clustering.centroid_assignment.coalesce().to(opt["device_id"])
    clustering.finetune_assignment = clustering.finetune_assignment.coalesce().to(opt["device_id"])
    clustering.expanded_norm_adj_graph = clustering.expanded_norm_adj_graph.coalesce().to(opt["device_id"])
